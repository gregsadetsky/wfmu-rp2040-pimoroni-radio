...
AudioOutputPimoroni::AudioOutputPimoroni() {
  Serial.println("AudioOutputPimoroni class init");
}

AudioOutputPimoroni::~AudioOutputPimoroni()
{
  Serial.println("AudioOutputPimoroni destroy");
  stop();
}

bool AudioOutputPimoroni::SetRate(int hz)
{
  Serial.println("AudioOutputPimoroni setrate");
  Serial.println(hz);
...
bool AudioOutputPimoroni::SetBitsPerSample(int bits)
{
  Serial.println("SetBitsPerSample");
  Serial.println(bits);
...
bool AudioOutputPimoroni::SetChannels(int channels)
{
  Serial.println("SetChannels");
...
bool AudioOutputPimoroni::begin(bool txDAC)
{
  Serial.println("begin");

  i2s.setBuffers(5,2048);
  i2s.setBCLK(pBCLK);
  i2s.setDATA(pDOUT);
...
int consumeSampleCounter = 0;
bool AudioOutputPimoroni::ConsumeSample(int16_t sample[2])
{
  i2s.write(sample[0]);
  i2s.write(sample[1]);
  return true;
...
void AudioOutputPimoroni::flush()
{
